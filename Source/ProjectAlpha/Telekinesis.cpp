...
bool UTelekinesis::RayCastObjects()
{
    ...
    const FCollisionObjectQueryParams ObjectType = FCollisionObjectQueryParams(ECollisionChannel::ECC_PhysicsBody);
    ...
    GetWorld()->SweepMultiByObjectType(HitResults,CharacterLocation ,CharacterLocation, Rotation, ObjectType, CollisionShape, ExtraParams);
   
	for (const auto& HitResult : HitResults)
	{
		const auto ActorHit = HitResult.GetActor();
		if (!ensure(ActorHit)) return false;	
		if (ActorHit)
		{
...
void UTelekinesis::GetAudioComponent()
{
	
	//if (!ensure(TelekinesisAudioCue))
	//{
		//UE_LOG(LogTemp, Warning, TEXT("Audio Handler Not Found"));
	//}
